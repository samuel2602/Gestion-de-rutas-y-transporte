...
## 15. Frontend y conexión con la base de datos

El proyecto incluye una interfaz web para usar la aplicación sin depender de Swagger:

- `index.html`: estructura de la interfaz.
- `style.css`: estilos responsive del panel de gestión.
- `app.js`: consultas `GET`, creación `POST`, actualización `PUT` y eliminación `DELETE` hacia la API.

La conexión sigue esta ruta:

```text
Navegador (frontend) -> FastAPI (main.py y routers/) -> SQLAlchemy -> PostgreSQL de Supabase
```

El navegador **no se conecta directamente** a Supabase ni conoce `DATABASE_URL`. Esto protege las credenciales: FastAPI recibe la solicitud, valida los datos con Pydantic y usa `database.py` para guardar o consultar los registros en Supabase.

La conexión se verificó con `GET /prueba-db`, que ejecuta `SELECT 1` y debe responder:
...
La forma recomendada es servir todo desde FastAPI:
...
- Prueba de conexión: `http://127.0.0.1:8000/prueba-db`

También se permite usar Live Server en el puerto `5500`. En ese caso `app.js` consulta la API local en `http://127.0.0.1:8000`; la configuración CORS de `main.py` autoriza ese origen.

### Subir el proyecto a GitHub y desplegarlo

Subir el código a GitHub **no ejecuta FastAPI**. El enlace del repositorio solo muestra archivos y no podrá mostrar los datos de Supabase. GitHub Pages tampoco puede ejecutar Python: puede alojar HTML, CSS y JavaScript, pero no el backend.

Para que el frontend, la API y los datos funcionen sin encender tu computador, despliega este repositorio en un servicio que ejecute Python, por ejemplo Render, Railway o Fly.io. Como FastAPI sirve `index.html`, `style.css` y `app.js`, basta desplegar un único servicio: al abrir la URL asignada por ese servicio funcionarán tanto el panel como la API y `/docs`.

Configuración típica de Render/Railway:
...
En el panel del hosting agrega una variable de entorno llamada `DATABASE_URL` con la misma cadena de conexión de Supabase que usas localmente. No subas `.env`: ya está excluido en `.gitignore`.

Después del despliegue, comprueba primero `https://tu-dominio/prueba-db`. Si responde correctamente, entra a `https://tu-dominio/`: el panel mostrará y guardará los mismos datos de Supabase.

> Si eliges GitHub Pages solo para el frontend, tendrás que desplegar la API por separado, configurar en `app.js` la URL pública de esa API y autorizar el dominio de GitHub Pages en CORS. Para este proyecto es más sencillo y seguro desplegar frontend y backend juntos.
